...
const WIDTH = 5;              // Equation format: NUM OP NUM OP NUM (5 tokens)
const targets = [20, 40, 60, 80, 100];  // Possible target values to hit
const numberPool = [1, 2, 5, 10, 12, 15, 20, 25, 35, 60, 80, 100]; // Available numbers to choose from
...
window.onload = function () { // Run this when the page loads
  ...
  document.getElementById("choices").innerText =
    "Equation pattern: NUM OP NUM OP NUM"; // Show hint
...
function chooseSecretEquation() {
  ...
  const candidates = []; // List of valid equations for target

  // Generate all possible NUM OP NUM OP NUM from pool
  for (const a of numberPool) {
    for (const b of numberPool) {
      for (const c of numberPool) {
        for (const op1 of operators) {
          for (const op2 of operators) {
            const expr = `${a}${op1}${b}${op2}${c}`; // Build equation
            try {
              const val = eval(expr);  // Evaluate it
              if (!isFinite(val)) continue; // Skip invalid (like division by 0)
              if (Math.abs(val - TARGET) < 1e-9) { // If matches target
                candidates.push([String(a), op1, String(b), op2, String(c)]); // Save tokens
              }
            } catch (e) {
  ...
  // Pick random secret equation from candidates
  if (candidates.length > 0) {
    secretTokens = candidates[Math.floor(Math.random() * candidates.length)];
  } else {
    // Fallback if none found
    secretTokens = ["10", "+", "10", "+", "0"];
  }

  console.log(`Secret for ${TARGET}:`, secretTokens.join("")); // Debug in console
}
